...
class BaseInputSimulation:
    ...
    def base_xml_headless_simulation(
        ...
        xml_file.write(
            '<Simulation id="1" sourcePath="'
            + dengue_model_filename
            + '" finalStep="2" experiment="explore_model">'
        )
        xml_file.write("<Parameters>\n")
        xml_file.write(
            '<Parameter var="build_filename" type="STRING" value="'
            + build_filename
            + '" />\n'
        )
        xml_file.write(
            '<Parameter var="road_filename" type="STRING" value="'
            + road_filename
            + '" />\n'
        )
        xml_file.write(
            '<Parameter var="shape_filename" type="STRING" value="'
            + shape_filename
            + '" />\n'
        )
        xml_file.write(
            '<Parameter var="mosquitoes_data_filename_inp" type="STRING" value="'
            + mosquitoes_filename
            + '" />\n'
        )
        xml_file.write(
            '<Parameter var="human_data_filename_inp" type="STRING" value="'
            + people_filename
            + '" />\n'
        )
        xml_file.write(
            '<Parameter var="mosquitoes_data_filename_otp" type="STRING" value="'
            + mosquitoes_output
            + '" />\n'
        )
        xml_file.write(
            '<Parameter var="human_data_filename_otp" type="STRING" value="'
            + people_output
            + '" />\n'
        )
        xml_file.write("</Parameters>\n")

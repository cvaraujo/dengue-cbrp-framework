    ...
    def _prepare_environment(self):
        try:
            head = {
                ...
                "model": "simulation/models/" + self._model,
            ...
            parameters = {
                ...
                "mosquitoes_csv_filename": (
                    "STRING",
                    path.join(
                        "temp/simulation_states", "mosquitoes_" + self._id + ".csv"
                    ),
                ),
                "people_csv_filename": (
                    "STRING",
                    path.join("temp/simulation_states", "people_" + self._id + ".csv"),
                ),
                "outbreaks_csv_filename": (
                    "STRING",
                    path.join(
                        "temp/simulation_states", "outbreaks_" + self._id + ".csv"
                    ),
                ),
                "mosquitoes_csv_filename_output": (
                    "STRING",
                    path.join(
                        "temp/simulation_states", "mosquitoes_" + self._id + 1 + ".csv"
                    ),
                ),
                "people_csv_filename_output": (
                    "STRING",
                    path.join(
                        "temp/simulation_states", "people_" + self._id + 1 + ".csv"
                    ),
                ),
                "outbreaks_csv_filename_output": (
                    "STRING",
                    path.join(
                        "temp/simulation_states", "outbreaks_" + self._id + 1 + ".csv"
                    ),
                ),
            }
            self._headless_file = path.join(
                "temp/simulation_states", self._headless_file + "_" + self._id + ".xml"
            )

...
class StochasticInstanceGenerator:
    ...
    def _prepare_parameters(self, shp_path, exec_id, start_date, max_cycles, save_states):
        return {
            "sqlite_ds": ("string", ""),
    ...
    def _write_prob_from_scenarios(self, scenarios: List, graph: Graph, output_name: str):
        """
            ...
            for i, scenario in enumerate(scenarios):
                ...
                for b in range(block_count):
                    ...
                    if count > 0:
                        ...
                        raise ValueError(f"[Erro] Bloco {b} no cenário {i} tem número negativo de casos!")

    def generate(self, city: str, map_size: int, start_date: str, end_date: str, exec_id: int, people_per_km2: float, num_scenarios: int):
        print("[INFO] Connecting to database...")
        self.db.clear_database()

        print(f"[INFO] Loading OSM map: {city} ({map_size})")
        osm = OpenStreetMap(city, map_size)
        graph = MapAdapter.convert_osm_to_graph(osm, True)

        city_key, city_file = self._get_city_info(city)

        print("[INFO] Retrieving dengue cases...")
        cases = self.db.get_notifications_between_dates(start_date, end_date, city_key)

        print("[INFO] Processing blocks and population data...")
        ...
        print("[INFO] Inserting starting scenario...")
        self.db.create_starting_scenario(
        ...
        print("[INFO] Running initial simulation...")
        params = self._prepare_parameters(shp_path, exec_id, start_date, max_cycles=0, save_states=False)
        sim: Simulation = Simulation()
        sim.run_simulation(JsonAdapter.convert_param_2_list(params), is_batch=False)

        print("[INFO] Running batch simulation...")
        params = self._prepare_parameters(shp_path, exec_id, start_date, max_cycles=14, save_states=True)
        sim.run_simulation(JsonAdapter.convert_param_2_list(params), is_batch=True)

        print("[INFO] Exporting instance...")
        instance_file = os.path.join(self.output_folder, f"{city_file}-{map_size}-{exec_id}.txt")
        self._write_instance(graph, instance_file, infected)

        print("[INFO] Extracting and aggregating simulated cases...")
        sim_cases = self.db.query(
            f"SELECT * FROM people WHERE execution_id = {exec_id} AND state >= 1"
        ).drop_duplicates(subset=["id"])

        grouped = sim_cases.groupby(["living_place", "simulation_id"], as_index=False)["state"].sum()

        all_scenarios = [
            grouped[grouped["simulation_id"] == s]
            .set_index("living_place")["state"]
            .reindex(range(graph.b), fill_value=0)
            .tolist()
            for s in range(num_scenarios)
        ]

        print("[INFO] Saving scenario probabilities...")
        scenario_file = os.path.join(self.output_folder, f"scenarios-{city_file}-{map_size}-{exec_id}.txt")
        self._write_prob_from_scenarios(all_scenarios, graph, scenario_file)

        self.db.clear_database()
        self.db.close()
        print("[INFO] Stochastic instance generation completed.")
